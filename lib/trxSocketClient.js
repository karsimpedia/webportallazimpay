...
async function forwardToPush(payload, eventName) {
  try {
    ...
    console.log(payload);
    let msg = `Trx  ${payload.productCode} ke ${payload.msisdn} harga ${payload.sellPrice} status ${payload.status} `;
    const title = `Transaksi ${payload.status}`;
    const pesan = msg;

    const pushUrl =  process.env.WEBPORTAL_PUSH_URL || `http://127.0.0.1:${PORT}/api/sendpush`;
    const body = {
      appid,
      title,
      pesan,
